...
DataPrep1 <- get(load('../1-download_preprocessing/ESCA_PreprocessedData_all_Prep_08072019.rda'))
DataFilt <- get(load('../1-download_preprocessing/ESCA_PreprocessedData_all_Filt_08072019.rda'))
...
write.table(dataDEGs.TSS, file = "DEA_TSS_ESCA.txt", quote = FALSE)
...
dataDEGs.Plate <- TCGAanalyze_DEA(mat1 = dataFilt[,dataSmNT_p],
                                ...
                                voom = TRUE)
#9610
write.table(dataDEGs.Plate, file = "DEA_Plate_ESCA.txt", quote = FALSE)
...
write.table(dataDEGs.Center, file = "DEA_Center_ESCA.txt", quote = FALSE)
