...
setwd("/data/user/shared_projects/ulk1/tcga/rnaseq/GBM/2-DEA/")
...
dataframe_GBM <- get(load("../1-download_preprocessing/GBM_PreprocessedData_all_Filt_07062019.rda"))
my_IDs <- get_IDs(dataframe_GBM)
length(which(my_IDs$condition=="cancer"))
length(which(my_IDs$condition=="normal"))
limma_name <- "limma_GBM_all_tss.csv"
up_name <- "up_limma_GBM_tss.txt"
down_name <- "down_limma_GBM_tss.txt"

limma_tss(my_IDs,dataframe_GBM,limma_name,up_name,down_name)

limma_GBM <- read.csv("limma_GBM_all_tss.csv")


#5131 up / 4149 down

limma.TSS.ULK1 <- subset(limma_GBM, X =="ENSG00000177169")
limma.TSS.ULK2 <- subset(limma_GBM, X== "ENSG00000083290")
...
edgeR_name <- "edgeR_GBM_all.csv"
up_name <- "up_edgeR_GBM.txt"
down_name <- "down_edgeR_GBM.txt"

edgeR_tss(dataframe_GBM,edgeR_name,up_name,down_name)

#5939 up / 3837 down

edgeR.GBM <- read.csv("edgeR_GBM_all.csv")

ensemblIDs <- c("ENSG00000177169", #ULK1
                "ENSG00000083290" #ULK2)
                
edgeR.TSS.ULK1 <- subset(edgeR.GBM, X =="ENSG00000177169")
edgeR.TSS.ULK2 <- subset(edgeR.GBM, X== "ENSG00000083290")
