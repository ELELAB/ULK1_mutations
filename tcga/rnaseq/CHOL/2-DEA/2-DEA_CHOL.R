...
setwd("/data/user/shared_projects/ulk1/tcga/rnaseq/CHOL/2-DEA/")
...
DataPrep1 <- get(load('../1-download_preprocessing/CHOL_PreprocessedData_all_Prep_11062019.rda'))
DataFilt <- get(load('../1-download_preprocessing/CHOL_PreprocessedData_all_Filt_11062019.rda'))
...
write.table(dataDEGs.TSS, file = "DEA_TSS_CHOL.txt", quote = FALSE)
...
dataDEGs.Plate <- TCGAanalyze_DEA(mat1 = dataFilt[,dataSmNT_p],
                                ...
                                voom = TRUE)
#9610
write.table(dataDEGs.Plate, file = "DEA_Plate_CHOL.txt", quote = FALSE)
...
write.table(dataDEGs.Center, file = "DEA_Center_CHOL.txt", quote = FALSE)
