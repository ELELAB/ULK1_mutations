...
dataframe_BRCA <- get(load("../1-download_preprocessing/BRCA_PreprocessedData_all_Filt_11062019.rda"))
my_IDs <- get_IDs(dataframe_BRCA)
length(which(my_IDs$condition=="cancer"))
length(which(my_IDs$condition=="normal"))
limma_name <- "limma_BRCA_all_tss.csv"
up_name <- "up_limma_BRCA_tss.txt"
down_name <- "down_limma_BRCA_tss.txt"

limma_tss(my_IDs,dataframe_BRCA,limma_name,up_name,down_name)

limma_BRCA <- read.csv("limma_BRCA_all_tss.csv")

#4779 up / 4206 down

limma.TSS.ULK1 <- subset(limma_BRCA, X =="ENSG00000177169")
limma.TSS.ULK2 <- subset(limma_BRCA, X== "ENSG00000083290")
...
edgeR_name <- "edgeR_BRCA_all.csv"
up_name <- "up_edgeR_BRCA.txt"
down_name <- "down_edgeR_BRCA.txt"

edgeR_tss(dataframe_BRCA,edgeR_name,up_name,down_name)

#6549 up /3151  down

edgeR.BRCA <- read.csv("edgeR_BRCA_all.csv")

edgeR.TSS.ULK1 <- subset(edgeR.BRCA, X =="ENSG00000177169")
edgeR.TSS.ULK2 <- subset(edgeR.BRCA, X== "ENSG00000083290")
