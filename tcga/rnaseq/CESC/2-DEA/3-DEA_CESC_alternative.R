...
setwd("/data/user/shared_projects/ulk1/tcga/rnaseq/CESC/2-DEA/")
...
dataframe_CESC <- get(load("../1-download_preprocessing/CESC_PreprocessedData_all_Filt_08072019.rda"))
my_IDs <- get_IDs(dataframe_CESC)
length(which(my_IDs$condition=="cancer"))
length(which(my_IDs$condition=="normal"))
limma_name <- "limma_CESC_all_tss.csv"
up_name <- "up_limma_CESC_tss.txt"
down_name <- "down_limma_CESC_tss.txt"

limma_tss(my_IDs,dataframe_CESC,limma_name,up_name,down_name)

limma_CESC <- read.csv("limma_CESC_all_tss.csv")

#1669 up / 2158 down

limma.TSS.ULK1 <- subset(limma_CESC, X =="ENSG00000177169")
limma.TSS.ULK2 <- subset(limma_CESC, X== "ENSG00000083290")
...
edgeR_name <- "edgeR_CESC_all.csv"
up_name <- "up_edgeR_CESC.txt"
down_name <- "down_edgeR_CESC.txt"

edgeR_tss(dataframe_CESC,edgeR_name,up_name,down_name)

#2134 up /1966  down

edgeR.CESC <- read.csv("edgeR_CESC_all.csv")

edgeR.TSS.ULK1 <- subset(edgeR.CESC, X =="ENSG00000177169")
edgeR.TSS.ULK2 <- subset(edgeR.CESC, X== "ENSG00000083290")
