...
setwd("/data/user/shared_projects/ulk1/tcga/rnaseq/COAD/2-DEA/")
...
dataframe_COAD <- get(load("../1-download_preprocessing/COAD_PreprocessedData_all_Filt_08072019.rda"))
my_IDs <- get_IDs(dataframe_COAD)
length(which(my_IDs$condition=="cancer"))
length(which(my_IDs$condition=="normal"))
limma_name <- "limma_COAD_all_tss.csv"
up_name <- "up_limma_COAD_tss.txt"
down_name <- "down_limma_COAD_tss.txt"

limma_tss(my_IDs,dataframe_COAD,limma_name,up_name,down_name)

limma_COAD <- read.csv("limma_COAD_all_tss.csv")


#5131 up / 4149 down

limma.TSS.ULK1 <- subset(limma_COAD, X =="ENSG00000177169")
limma.TSS.ULK2 <- subset(limma_COAD, X== "ENSG00000083290")
...
edgeR_name <- "edgeR_COAD_all.csv"
up_name <- "up_edgeR_COAD.txt"
down_name <- "down_edgeR_COAD.txt"

edgeR_tss(dataframe_COAD,edgeR_name,up_name,down_name)

#5939 up / 3837 down

edgeR.COAD <- read.csv("edgeR_COAD_all.csv")

ensemblIDs <- c("ENSG00000177169", "ENSG00000083290" )
                
edgeR.TSS.ULK1 <- subset(edgeR.COAD, X =="ENSG00000177169" )
edgeR.TSS.ULK2 <- subset(edgeR.COAD, X== "ENSG00000083290" )
