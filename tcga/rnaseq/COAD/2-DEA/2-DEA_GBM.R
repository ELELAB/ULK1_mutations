...
DataPrep1 <- get(load('../1-download_preprocessing/GBM_PreprocessedData_all_Prep_07062019.rda'))
DataFilt <- get(load('../1-download_preprocessing/GBM_PreprocessedData_all_Filt_03062019.rda'))
...
write.table(dataDEGs.TSS, file = "DEA_TSS_GBM.txt", quote = FALSE)
...
dataDEGs.Plate <- TCGAanalyze_DEA(mat1 = dataFilt[,dataSmNT_p],
                                ...
                                voom = TRUE)
#9610
write.table(dataDEGs.Plate, file = "DEA_Plate_GBM.txt", quote = FALSE)
...
write.table(dataDEGs.Center, file = "DEA_Center_GBM.txt", quote = FALSE)
