### AIM ###
#DEA on RNASEQ data without tcB;abiolinks functions#

### WORKING DIRECTORY ####
#replace with proper location #
setwd("/data/user/shared_projects/ulk1/tcga/rnaseq/BLCA/2-DEA/")
...
dataframe_BLCA <- get(load("../1-download_preprocessing/BLCA_PreprocessedData_all_Filt_08072019.rda"))
my_IDs <- get_IDs(dataframe_BLCA)
length(which(my_IDs$condition=="cancer"))
length(which(my_IDs$condition=="normal"))
limma_name <- "limma_BLCA_all_tss.csv"
up_name <- "up_limma_BLCA_tss.txt"
down_name <- "down_limma_BLCA_tss.txt"

limma_tss(my_IDs,dataframe_BLCA,limma_name,up_name,down_name)

limma_BLCA <- read.csv("limma_BLCA_all_tss.csv")


#5131 up / 4149 down

limma.TSS.ULK1 <- subset(limma_BLCA, X =="ENSG00000177169")
limma.TSS.ULK2 <- subset(limma_BLCA, X== "ENSG00000083290")
...
edgeR_name <- "edgeR_BLCA_all.csv"
up_name <- "up_edgeR_BLCA.txt"
down_name <- "down_edgeR_BLCA.txt"

edgeR_tss(dataframe_BLCA,edgeR_name,up_name,down_name)

#5939 up / 3837 down

edgeR.BLCA <- read.csv("edgeR_BLCA_all.csv")

ensemblIDs <- c("ENSG00000177169","ENSG00000083290")
                
edgeR.TSS.ULK1 <- subset(edgeR.BLCA, X =="ENSG00000177169" )
edgeR.TSS.ULK2 <- subset(edgeR.BLCA, X== "ENSG00000083290" )
